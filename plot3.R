exploreDataPlot3 <- function() {
    ...
    # Create plot3, a scatterplot (line type) of all three Energy sub metering values
    # vs. Time
    # plot is 480x480 pixel, PNG type
    png("plot3.png",width=480,height=480,units="px", bg="transparent", pointsize=12)
    with(housePower,plot(dateTime,Sub_metering_1,ann=FALSE,type="l",col="black",lwd=1))
    with(housePower,lines(dateTime,Sub_metering_2,col="red",lwd=1))
    with(housePower,lines(dateTime,Sub_metering_3,col="blue",lwd=1))
